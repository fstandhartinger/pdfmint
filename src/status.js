...
async function snapshot() {
  ...
  return {
    ...
    last_7d: {
      documents: w.n,
      succeeded: w.ok,
      success_rate: w.n ? Number(((w.ok / w.n) * 100).toFixed(2)) : null,
    },
...
<main class="dash">
  ...
  <section class="card">
    ...
    <table class="rows">
      ...
      <tr><th>Succeeded / failed</th><td>${num(s.last_24h.succeeded)} / ${num(s.last_24h.failed)}</td></tr>
      <tr><th>Success rate</th><td>${s.last_24h.success_rate === null ? '—' : `${s.last_24h.success_rate}%`}</td></tr>
    ...
    <p class="muted">One bar per hour over the last 24 hours, tallest bar = ${peak.toLocaleString('en-US')} document${peak === 1 ? '' : 's'}. Red means at least one document failed in that hour. Hover a bar for the exact count.</p>
  </section>
  ...
  <section class="card">
    ...
    <table class="rows">
      ...
      <tr><th>Success rate</th><td>${s.last_7d.success_rate === null ? '—' : `${s.last_7d.success_rate}%`}</td></tr>
    </table>
  </section>
  ...
  <section class="card">
    ...
    <ul>
      <li>No third-party uptime monitor yet. The numbers above are self-reported.</li>
